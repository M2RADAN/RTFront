...
const Edit = () => {
	...
	const { isPending, routeInfo, errorMessage } = useAppSelector((store) => store.route);
	...
	if (!notes?.length && isAuth) return <div className='temp'>Выбери, точки. Пожалуйста</div>;
	if (errorMessage) return <p>{errorMessage}</p>;
	// card preview
	return isPending && isAuth ? (
		<div className='temp'>Загрузка...</div>
	) : isAuth ? (
		<div className='temp'>
			<p>Время пути:</p>
